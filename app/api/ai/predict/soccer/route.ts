  ...
  try {
    ...
    if (gameData.length) {
      const PROP_KEYS: Record<string, string[]> = {
        ...
        'Passes Completed': ['accuratePass', 'totalPass'],
        'Tackles': ['totalTackle', 'wonTackle'],
        'Dribbles': ['wonContest', 'duelWon'],
      };
      ...
      gamesStr = `LAST ${Math.min(gameData.length, 10)} MATCHES — ${propType} | HIT RATE: ${overCount}/${Math.min(gameData.length, 10)} hit Over ${line}:\n${gameLines}`;
    }
    ...
    if (seasonContext?.context && typeof seasonContext.context === 'object') {
      ...
      const keys = ['totalShots', 'onTargetScoringAttempt', 'goals', 'goalAssist', 'keyPass', 'totalTackle'];
      const rows = keys.filter((k) => os[k] != null).slice(0, 6).map((k) => {
        const ov = os[k] != null ? (os[k] / oApps).toFixed(2) : '—';
        const hv = hs[k] != null ? (hs[k] / hApps).toFixed(2) : '—';
        const av = as_[k] != null ? (as_[k] / aApps).toFixed(2) : '—';
        return `  ${label(k)}: ${ov}/game overall | ${hv}/game home | ${av}/game away`;
      }).join('\n');
      if (rows) {
        seasonStr = `SEASON STATS (${ctx.season || ''} — ${ctx.tournament || ''}, ${oApps} appearances):\n${rows}`;
      }
    }

    const prompt = `You are Pick Prophet — a Soccer prop betting model analyzing ${league || 'football'} props. Use all available data to make a sharp prediction.
...
Respond ONLY in valid JSON:
{
  ...
  "confidence": 1-100,
  "reasoning": "4-6 sentences citing specific stats, home/away splits, and recent form.",
  ...
  "data_confidence": "Strong" or "Moderate" or "Limited"
    ...
    console.error('Soccer predict error:', err);
